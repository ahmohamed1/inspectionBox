...
class HyperParameterForm(FlaskForm):
    select_model = StringField('select_model', validators=[validators.InputRequired()])
    batch_size = IntegerField('batch_size', validators=[validators.InputRequired()])
...
def train_page():
    form = HyperParameterForm(meta={'csrf': False})
